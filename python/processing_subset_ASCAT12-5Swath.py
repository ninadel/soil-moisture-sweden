...
Date: 6/27/2020
Script for selecting ASCAT 12.5 H101 bands and exporting them to a new netCDF file
"""
import json
import netCDF4
import numpy
...
input_dir = r"D:\sm_backup\ascat-h101-12.5-km\2018_nc"
output_dir = r"D:\sm_backup\ascat-h101-12.5-km\select_bands\2018_nc"
product_name = "ASCAT 12.5 Swath"
...
meta_variables = ['lat', 'lon', 'soil_moisture', 'proc_flag', 'frozen_soil_probability']
...
for file in os.listdir(input_dir):
    ...
    dst_filename = file[:file_extension_index] + "_bnd" + file[file_extension_index:]
    with netCDF4.Dataset(os.path.join(input_dir, file)) as src, netCDF4.Dataset(os.path.join(output_dir, dst_filename),
        ...
        for variable in meta_variables:
            ...
            if '_FillValue' in src_var_attributes.keys():
