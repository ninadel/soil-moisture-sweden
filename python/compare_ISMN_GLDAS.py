...
it then finds the nearest GLDAS grid point and reads the GLDAS data. After temporal matching
?? and scaling using linear CDF matching ??
it computes several metrics, like the correlation coefficients(Pearson's, Spearman's and Kendall's), Bias,
...
for station in ISMN_reader.stations_that_measure('soil moisture'):
    ...
    for ISMN_time_series in station.data_for_variable('soil moisture', min_depth=0, max_depth=0.1):
        ...
        # print(ISMN_time_series.data.head())
        ascat_time_series.data.to_csv(os.path.join(test_output_folder,r'ascat_time_series.csv'), index=True, header=False, sep=" ")
        ISMN_time_series.data.to_csv(os.path.join(test_output_folder,r'ISMN_time_series.csv'), index=True, header=False, sep=" ")
