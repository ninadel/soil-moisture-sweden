...
eval_data_reader = SPL4SMP_nc_Ds(eval_data_folder, eval_grid_folder,
                                     grid_filename=eval_grid_file,
...
for station in ISMN_reader.stations_that_measure('soil moisture'):
    ...
    for ISMN_time_series in station.data_for_variable('soil moisture', min_depth=0, max_depth=0.1):
        ...
        eval_time_series.data = eval_time_series.data[label_eval]

        # drop nan values before doing any matching
        eval_time_series.data = eval_time_series.data.dropna()
        ...
        print(type(eval_time_series.data))
        print(type(ISMN_time_series.data))
        print('pause')
        print('eval_time_series.data.head()')
        print(eval_time_series.data.head())
        print('ISMN_time_series.data.head()')
        print(ISMN_time_series.data.head())
        matched_data = temp_match.matching(eval_time_series.data, ISMN_time_series.data,
                                                window=1 / 24.)
        # matched ISMN data is now a dataframe with the same datetime index
        # as eval_time_series.data and the nearest insitu observation
